/**
 * БСП — Telegram Bot v5.0.0 (VM) (Deno Deploy + grammy)
 * НОВОЕ в v4.6.0:
...
// ─── KV ───────────────────────────────────────────────────────────────────────
const kv = await Deno.openKv(Deno.env.get("KV_PATH") ?? "./data/kv.db");
...
Deno.serve({ port: Number(Deno.env.get("PORT") ?? 8000) }, async (req: Request) => {
  ...
  if (url.pathname === "/health") {
    ...
    return new Response(JSON.stringify({ status: "ok", version: "5.0.0", ts: new Date().toISOString(), crons: cronStatuses }),
  ...
  if (url.pathname === "/repair-index") {
    ...
    statsCache = null;
    return new Response(
      JSON.stringify({ ok: true, rebuilt: fixed, ts: new Date().toISOString() }),
  ...
  if (url.pathname === "/debug-kv") {
    ...
    const byStatus: Record<string, number[]> = {};
    for await (const entry of kv.list({ prefix: ["by_status"] })) {
      const status = entry.key[1] as string;
      const tgId = entry.key[2] as number;
      if (!byStatus[status]) byStatus[status] = [];
      byStatus[status].push(tgId);
    }
    const sessions: string[] = [];
    for await (const entry of kv.list({ prefix: ["session"] })) {
      sessions.push(String(entry.key[1]));
    }
    return new Response(
      JSON.stringify({ users, byStatus, sessions, ts: new Date().toISOString() }, null, 2),
  ...
  if (url.pathname === "/whereami") {
    ...
    const t = Date.now();
    await kv.set(["__wtest"], t);
    const back = (await kv.get<number>(["__wtest"], { consistency: "strong" })).value;
    let users = 0;
    for await (const _ of kv.list({ prefix: ["users"] }, { consistency: "strong" })) users++;
    return new Response(JSON.stringify({ region, deployment: dep, wrote: t, readback: back, persisted: back === t, users }),
...
log("INFO", "bot_started", { version: "5.0.0", mode: "vm-polling" });

// ── РЕЖИМ VM: LONG-POLLING ─────────────────────────────────────────────────────
// На одном сервере (VM) бот получает обновления через long-polling — публичный HTTPS
// не нужен. Данные — в локальной Deno KV (KV_PATH) на диске сервера: один экземпляр,
// полная согласованность, ничего не «исчезает». HTTP-сервер выше остаётся для /health.
await bot.api.deleteWebhook({ drop_pending_updates: false })
  .then(() => log("INFO", "webhook_deleted_for_polling", {}))
  .catch((e) => log("WARN", "delete_webhook_fail", { error: String(e) }));
bot.start({
  drop_pending_updates: false,
  allowed_updates: ["message", "callback_query", "inline_query"],
  onStart: (bi) => log("INFO", "polling_started", { username: bi.username }),
});
